...
export class GameSceneControls extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.add
      .text(width / 2, 180, "Keyboard Controls", {
        fontSize: "28px",
        color: "#00ff00",
        fontStyle: "bold",
      })
    ...
    keyboardInstructions.forEach((text, index) => {
      this.add
        .text(width / 2, 230 + index * 35, text, {
          fontSize: "20px",
          color: "#dddddd",
    ...
    this.add
      .text(width / 2, 400, "Touch / Screen Click Regions", {
        fontSize: "28px",
        color: "#00ff00",
        fontStyle: "bold",
      })
    ...
    touchInstructions.forEach((text, index) => {
      this.add
        .text(width / 2, 450 + index * 35, text, {
          fontSize: "20px",
          color: "#dddddd",
          fontFamily: "monospace",
        })
    ...
    const startText = this.add
      .text(width / 2, 620, "PRESS ANY KEY OR CLICK TO START", {
        fontSize: "24px",
        color: "#ffff00",
        fontStyle: "bold",
      })
